...
export function DatabaseManagerView() {
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-800 dark:text-slate-100 font-sans">
      
      {/* Modern Header Section */}
      <div className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-gradient-to-br from-slate-50 via-white to-blue-50/40 shadow-sm dark:border-slate-800 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950/20 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="h-2.5 w-2.5 rounded-full animate-pulse" style={{ backgroundColor: primaryColor }} />
            <span className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: primaryColor }}>System Operations</span>
          </div>
        ...
        <div className="flex gap-3">
          <button 
            ...
            disabled={loading}
            className="p-2.5 bg-white dark:bg-slate-900/50 hover:bg-slate-50 dark:hover:bg-slate-800/80 border border-slate-200 dark:border-slate-800 rounded-xl transition-all text-slate-500 dark:text-slate-400"
          >
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-6">
        <button
          ...
          className={cn(
            ...
            activeTab === "list" ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
          )}
        ...
        <button
          ...
          className={cn(
            ...
            activeTab === "add" ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
          )}
        ...
        <button
          ...
          className={cn(
            "pb-3 text-sm font-semibold transition-all relative",
            activeTab === "logs" ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
          )}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "list" && (
            <>
              ...
              >
                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Connection Profiles</span>
                
                {connections.length === 0 ? (
                  <div className="p-8 border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900/50 text-center text-slate-400 dark:text-slate-500 text-sm shadow-sm">
                    ...
                    {connections.map((conn) => {
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            "p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between gap-3 group bg-white dark:bg-slate-900/40 shadow-sm",
                            isActive
                              ? "shadow-md"
                          ...
                          style={{
                            ...
                            backgroundColor: isActive ? `rgba(${primaryRgb}, 0.03)` : undefined,
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className={cn(
                              ...
                              conn.type === "mysql" && "bg-cyan-500/10 text-cyan-500 border-cyan-500/10",
                              conn.type === "mongodb" && "bg-emerald-500/10 text-emerald-500 border-emerald-500/10",
                              conn.type === "postgresql" && "bg-indigo-500/10 text-indigo-500 border-indigo-500/10"
                            )}>
                              {conn.type === "mysql" && "My"}
                          ...
                          <span className={cn(
                            ...
                            conn.status === "connected" && "bg-emerald-500 shadow-md shadow-emerald-500/40",
                            conn.status === "disconnected" && "bg-slate-400 dark:bg-slate-600",
                            conn.status === "error" && "bg-rose-500 shadow-md shadow-rose-500/40"
                          )} />
                        </div>
                      );
                    })}
              ...
              >
                {selectedConnection ? (
                  <div className="border border-slate-200/80 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900 p-6 space-y-6 shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-5">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 uppercase">
                            {selectedConnection.type}
                          </span>
                          <span className="text-[11px] text-slate-400 dark:text-slate-500">ID: {selectedConnection.id}</span>
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-1.5">{selectedConnection.name}</h3>
                      </div>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            selectedConnection.role === "backup"
                              ? "bg-indigo-500/10 text-indigo-600 border-indigo-500/20 cursor-default"
                              : "bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:border-slate-300"
                          )}
                          style={{
                            backgroundColor: selectedConnection.role === "backup" ? `rgba(${primaryRgb}, 0.08)` : undefined,
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                      <div className="space-y-1">
                        ...
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedConnection.host}</span>
                      </div>
                      <div className="space-y-1">
                        <span className="text-slate-400 dark:text-slate-500 block text-xs font-medium uppercase tracking-wider">Port</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedConnection.port}</span>
                      </div>
                      <div className="space-y-1">
                        <span className="text-slate-400 dark:text-slate-500 block text-xs font-medium uppercase tracking-wider">Database / Schema</span>
                    ...
                    {selectedConnection.role === "backup" && (
                      <div className="rounded-xl border p-4 space-y-4" style={{ backgroundColor: `rgba(${primaryRgb}, 0.03)`, borderColor: `rgba(${primaryRgb}, 0.1)` }}>
                        <div className="flex items-center gap-2">
                          <Clock className="h-5 w-5" style={{ color: primaryColor }} />
                          <div>
                            <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm">Backup Scheduler</h4>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-t border-slate-200 dark:border-slate-800/80 pt-3 gap-2">
                          ...
                          <button
                            ...
                            disabled={loading}
                            className="px-3.5 py-1.5 text-white font-semibold text-xs rounded-lg flex items-center gap-1.5 transition-all shadow-sm"
                  ...
                  <div className="border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl h-80 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 p-6 bg-white dark:bg-slate-900/10 shadow-sm">
                    <Server className="h-12 w-12 text-slate-300 dark:text-slate-700 mb-3" />
                    <p className="text-sm font-semibold">No connection selected</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Choose a connection profile from the list to manage.</p>
                  </div>
                )}
              </motion.div>
            </>
          ...
          {activeTab === "add" && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: 15 }}
              className="lg:col-span-3 max-w-2xl mx-auto w-full border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl p-6 space-y-6 shadow-sm"
            >
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Configure credentials for MySQL, PostgreSQL or MongoDB connection.</p>
              </div>

              <form onSubmit={handleSaveConnection} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1 col-span-2">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Connection Display Name *</label>
                    <input
                      type="text"
                      required
                      value={name}
                      ...
                      className="w-full px-4 py-2 bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-200 text-sm focus:outline-none focus:ring-1 focus:ring-opacity-50 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600"
                      style={{ '--tw-ring-color': primaryColor } as React.CSSProperties}
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Database Engine *</label>
                    <select
                      value={type}
                      onChange={(e) => setType(e.target.value)}
                      className="w-full px-4 py-2 bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-200 text-sm focus:outline-none transition-all"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Host Address *</label>
                    <input
                      type="text"
                      required
                      value={host}
                      ...
                      className="w-full px-4 py-2 bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl text-slate-900 dark:text-slate-200 text-sm focus:outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Port *</label>
                    <input
                      type="number"
                      required
                      value={port}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Database / Schema Name *</label>
                    <input
                      type="text"
                      required
                      value={databaseName}
                  ...
                  <div className="space-y-1 col-span-2">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Connection URL Override (Alternative)</label>
                    <input
                      type="text"
                      value={connectionUrl}
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-slate-100 dark:border-slate-800">
                  <input
                    ...
                    style={{ color: primaryColor }}
                  />
                  <label htmlFor="ssl" className="text-xs font-semibold text-slate-500 dark:text-slate-400 cursor-pointer select-none">
                ...
                {testResult && (
                  ...
                  >
                    {testResult.success ? (
                      ...
                      <XCircle className="h-5 w-5 flex-shrink-0 text-rose-500 dark:text-rose-400" />
                    )}
                    <div>
                ...
                <div className="flex justify-end gap-3 pt-5 border-t border-slate-100 dark:border-slate-800">
                  <button
                    ...
                    className="px-4 py-2 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 bg-white dark:bg-slate-950 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 font-semibold text-sm rounded-xl transition-all shadow-sm"
                  >
          ...
          {activeTab === "logs" && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="lg:col-span-3 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl p-6 space-y-4 shadow-sm"
            >
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
                <h2 className="text-lg font-bold text-slate-900 dark:text-slate-200 flex items-center gap-2">
                  <Activity className="h-5 w-5" style={{ color: primaryColor }} /> Activity Sync Logs
                </h2>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Audit trail of background backup tasks executed by the synchronization service daemon.</p>
              </div>

              {syncLogs.length === 0 ? (
                <div className="p-12 text-center text-slate-400 dark:text-slate-500 text-sm border border-slate-200 dark:border-slate-800 border-dashed rounded-xl">
                  No sync records found in target audit trail.
                </div>
              ) : (
                <div className="overflow-hidden border border-slate-200 dark:border-slate-850 rounded-xl shadow-sm">
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody>
                      {syncLogs.map((log) => (
                        <tr key={log.id} className="border-b border-slate-100 dark:border-slate-800/80 hover:bg-slate-50/50 dark:hover:bg-slate-900/10 text-slate-700 dark:text-slate-300 last:border-0">
                          ...
                          <td className="py-3.5 px-4 text-slate-400 dark:text-slate-500 truncate max-w-xs font-mono">
