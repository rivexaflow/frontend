...
import { Globe, Trash2, ShieldCheck, Info } from "lucide-react";
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const errorMsg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || (err as Error).message || "Failed to load domain settings.";
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
