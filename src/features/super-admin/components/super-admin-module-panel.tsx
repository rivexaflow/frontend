...
export function SuperAdminModulePanel({ title, description, bullets }: Props) {
  const metrics = bullets.slice(0, 3).map((text, idx) => ({
    label: `Control point ${idx + 1}`,
    value: text.split(":")[0]?.trim() ?? text,
    hint: text.includes(":") ? text.split(":").slice(1).join(":").trim() : "Platform signal"
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-[var(--rvx-midnight)]/10 bg-[var(--rvx-white)] p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-[var(--rvx-midnight)]">Platform notes</h2>
